...
BASEP="/xrootd/store/user/wjang/ttbar_pythia_limitCylinder_noVtxSmearing"
BASEX="/xrd/store/user/wjang/ttbar_pythia_limitCylinder_noVtxSmearing"
...
for i in {51..1050}; do
    condor_submit prod_gen_pythia_limitCylinder_noVtxSmearing.jds -batch-name "GEN_${PROCESS}" -append "output = log/runCMS-out_${PROCESS}_${i}.txt" -append "error = log/runCMS-error_${PROCESS}_${i}.txt" -append "log = log/runCMS-log_${PROCESS}_${i}.txt" -append "arguments = $BASE $PROCESS $i $XRDCP"
done
